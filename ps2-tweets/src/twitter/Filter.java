...
//       throw new RuntimeException("not implemented");
        List<Tweet> tweetWithKeyWord = new ArrayList<>();
        if (words.size() == 0) throw new RuntimeException("should contain key words");
        List<String> wordsToLowerCase = new ArrayList<>();
        for (String w: words) {
           wordsToLowerCase.add(w.toLowerCase());
        }
        for (Tweet tweet: tweets) {
            String [] textWords = tweet.getText().replaceAll("[^\\w+\\-]", " ").split(" ");
            for (String textWord: textWords) {
                if (wordsToLowerCase.contains(textWord.toLowerCase())) {
